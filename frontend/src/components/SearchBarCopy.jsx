...
function SearchBarCopy() {
    ...
    useEffect(() => {
      if (!stockNames || stockNames.length === 0) return; // ✅ Prevent errors on empty dataset

      if (query.length > 0) {
          const filtered = stockNames
              .filter(stock => stock && stock.symbol && stock.name) // ✅ Remove null values
              .filter(stock =>
          ...
          <div className="relative w-full">
            ...
            >
              {filteredResults.map((stock, index) => (
                            ...
                            ) : null // ✅ Skip rendering null items
                        ))}              
            </motion.ul>
          )}
          </div>
